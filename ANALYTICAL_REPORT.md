...
![Dashboard Screenshot](PoweBi_Dashboard/Screenshots/dashboard_demo.gif)
...
1.  **KPIs and Dynamic Filtering:**
    *   **Core Metrics:** The dashboard prominently displays high-level KPIs: **Total Routes, Total Stops,** and **Total Trips**.
    *   **Interactive Slicers:** Users can filter the entire report by **Mode of Transport** (Bus, Train, Ferry, Tram) and by **Day of the Week**. This feature allows for immediate comparison; for example, a user can instantly see that buses account for the overwhelming majority of total trips.

2.  **Geospatial Analysis:**
    *   **Interactive Map:** A map plots all stop locations, visually confirming **Hypothesis 3 (CBD Centrality)** by showing the high density of service in the urban core. This is more powerful than a static image as users can zoom and pan to explore specific areas of interest.

3.  **Hourly and Weekly Patterns:**
    *   **Trips by Hour:** A bar chart visualizing trips per hour validates **Hypothesis 4 (Commuter Peaks)**, clearly showing the bimodal surge in activity during morning and evening rush hours.
    *   **Weekday vs. Weekend Service:** A dedicated line chart compares trip volumes between weekdays and weekends, providing strong visual evidence for **Hypothesis 2 (Weekday Bias)**.

4.  **Performance and Hierarchy Analysis:**
    *   **Top 10 Routes:** A bar chart dynamically displays the most frequent routes. When filtered for "Bus," it confirms **Hypothesis 1 (Route Dominance)**. The dashboard intelligently uses route names for trains and route numbers for buses, enabling fair comparisons.
    *   **Most Busiest Stops:** A bar chart of the busiest stops (by activity count) provides further evidence for CBD centrality, identifying major interchanges like **King George Square Station** as the primary hubs of the network.
